...
void fft_default(CkFftContextBase* context, const CkFftComplex* input, CkFftComplex* output, int count, int stride, int expTableDiv)
{
    if (count == 4)
    {
    ...
    else if (count == 8)
    {
        ...
        const CkFftComplex* exp1 = context->expTable + expTableStride;
        const CkFftComplex* exp2 = exp1 + expTableStride;
        const CkFftComplex* exp3 = exp2 + expTableStride;
    ...
    else
    {
        ...
        while (out < outEnd)
        {
            fft_default(context, in, out, n, stride4, expTableDiv);
            in += stride;
            out += n;
        }
        ...
        else
        {
            ...
            fft_default(context, input, output, n, stride2);
            fft_default(context, input + stride, output + n, n, stride2);
...
void fft(CkFftContextBase* context, const CkFftComplex* input, CkFftComplex* output, int count, int expTableDiv)
{
    ...
    else
    {
        ...
        {
            fft_default(context, input, output, count, 1, expTableDiv);
        }
#else
        fft_default(context, input, output, count, 1, expTableDiv);
#endif
